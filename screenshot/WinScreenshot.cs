...
public class ScreenCapture
{
    ...
    public class User32
    {
        ...
        public static IDictionary<string, IntPtr> GetWindows()
        {
            ...
            EnumWindows(delegate (IntPtr wnd, IntPtr param)
            {
                ...
                try
                {
                    windows.Add(builder.ToString(), wnd);
                }
                catch (ArgumentException)
                {
                    Console.WriteLine("Already contains " + builder.ToString());
                }
        ...
        /// <summary> Lists all windows</summary>
        public static IEnumerable<String> ListParsableWindows()
        {
            var shellWindow = GetShellWindow();
            ...
            EnumWindows(delegate (IntPtr wnd, IntPtr param)
            {
                ...
                GetWindowText(wnd, builder, length + 1);
                builder.Append(";").Append(wnd.ToString());
        ...
        public static IntPtr FindMatchingWindow(string name)
        {
            var shellWindow = GetShellWindow();
            List<String> windows = new List<String>();
    ...
    public class MonitorInfo
    {
        ...
        public MonitorInfo(MonitorInfoWithHandle mih)
        {
            deviceName = mih.MonitorInfo.DeviceName;
            top = mih.MonitorInfo.Monitor.top;
            right = mih.MonitorInfo.Monitor.right;
            bottom = mih.MonitorInfo.Monitor.bottom;
            left = mih.MonitorInfo.Monitor.left;
            dpiScale = mih.DpiScale;
        }
    }
    ...
    {
        List<MonitorInfoWithHandle> mihs = GetMonitors();
        List<MonitorInfo> mons = new List<MonitorInfo>();
        foreach (var mih in mihs)
        {
            mons.Add(new ScreenCapture.MonitorInfo(mih));
        }
        return mons;
    }
    ...
    {
        var windows = User32.ListParsableWindows();
...
public class Startup
{
    public async Task<object> Invoke(dynamic input)
    {
        ...
        string command = (string)input.cmd;

        ScreenCapture sc = new ScreenCapture();
        if (command.Equals("listWindows"))
        {
            return ScreenCapture.User32.GetWindows();
            //return ScreenCapture.User32.ListParsableWindows();
        }
        else if (command.Equals("getWindow"))
        {
            return ScreenCapture.User32.FindMatchingWindow(input.name);
        }
        else if (command.Equals("screenshotWindowName"))
        {
            IntPtr wndHandle;
            ...
            {
                wndHandle = ScreenCapture.User32.FindMatchingWindow(windowName);
            }
            ...
            {
                Console.WriteLine("saving to file" + input.file);
                string fileName = (string)input.file + ".png";
        ...
        else if (command.Equals("listMonitors"))
        {

            List<ScreenCapture.MonitorInfo> mis = ScreenCapture.GetMonitorInfo();
            return mis;
        }
